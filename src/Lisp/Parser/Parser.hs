...
import Error.ErrorList (invalidRestrictedKeywordUse, lambdaArgError, parsingError)
...
parseLambdaArgs (S.Value value : _) = Error lambdaArgError $ "Unexpected value " ++ show value ++ ". Expected symbol."
parseLambdaArgs (S.List _ : _) =
    Error lambdaArgError $
