...
parseLambdaArgs (S.Value value:_) = Error lambdaArgError $ 
    "Unexpected value " ++ show value ++ ". Expected symbol."
parseLambdaArgs (S.List _:_) = Error lambdaArgError 
    $ "Unexpected list. Expected symbol."
...
parseSExpr (S.List [S.Symbol "define", S.List (S.Symbol name:args), sContent])
  | name `elem` restrictedKeyword = Error invalidRestrictedKeywordUse name
  | otherwise = A.Define name <$> (A.Lambda <$> 
    parseLambdaArgs args <*> parseSExpr sContent)
parseSExpr (S.List (S.Symbol "define":_))
  = Error invalidRestrictedKeywordUse 
    "Usage: (define {<name> | (<name> <args>)} <query>)"
